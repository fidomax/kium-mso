...
TIMER_TYPE* TIMERS[TIMER_NUM ] = {&timer_0, &timer_1, &timer_2, &timer_3, &timer_4, &timer_5, &timer_6, &timer_7, &timer_8, &timer_9, &timer_10, &timer_11, &timer_12, &timer_13, &timer_14, &timer_15};

volatile TWI_TRANSFER TWI_packet;
//------------------------------------------------------------------------------
// Функции заполнения packet
//------------------------------------------------------------------------------
void FillTWIPacket(unsigned char* Data, unsigned int Bytes,
		unsigned char Status, unsigned char Mode, int Error)
{
...
//------------------------------------------------------------------------------
void FillCanPacket(CanTransfer *canTransfer, unsigned char number,
		unsigned char mailbox, unsigned int mode, unsigned int mask,
...
// Чтение TWI
//------------------------------------------------------------------------------
int TWI_Read(unsigned char address, unsigned int iadress, unsigned char* Data,
		unsigned int AmountBytes, int flag)
{
...
// Запись TWI
//------------------------------------------------------------------------------
int TWI_Write(unsigned char address, unsigned int iadress, unsigned char* Data,
		unsigned int AmountBytes, int flag)
{
		...
		unsigned int ByteNum, unsigned char BytesToWrite, unsigned char* Data)
{
	...
	if ((Mezonin < COUNT_MEZ )&& (PageNum <= MEZ_MEM_PAGES) && (ByteNum <= MEZ_MEM_PAGE_BYTES) && ((ByteNum + BytesToWrite) <= (MEZ_MEM_PAGE_BYTES+1))){
		...
		unsigned int ByteNum, unsigned char BytesToRead, unsigned char* Data)
{
	...
	if ((Mezonin < COUNT_MEZ )&& (PageNum <= MEZ_MEM_PAGES) && (ByteNum <= MEZ_MEM_PAGE_BYTES) && ((ByteNum + BytesToRead) <= (MEZ_MEM_PAGE_BYTES+1))){
...
void TWI_Transfer_Handler(void)
{
	...
	if ((TWI_status & AT91C_TWI_RXRDY)== AT91C_TWI_RXRDY) {
		...
		if ((TWI_status & AT91C_TWI_TXRDY)== AT91C_TWI_TXRDY) { // Ранее был код из 2/Mso_Functions
	...
	if ((TWI_status & AT91C_TWI_TXCOMP)== AT91C_TWI_TXCOMP) {
		if (((TWI_status & AT91C_TWI_NACK)== AT91C_TWI_NACK)) {
			...
			if (TWI_packet.TransferStatus == TWI_TRANSFER_FINISH ) {
